...
# Try importing core modules; capture the original error
try:
    ...
    _IMPORT_ERROR = str(e)
    # Define minimal stubs for help display only
...
def cmd_repl(args: argparse.Namespace) -> int:
    ...
    # Guard: if core imports failed, enter degraded REPL
    if not _SETUP_AVAILABLE:
        print_error(f"Core modules unavailable: {_IMPORT_ERROR}")
        print_info("Running in degraded mode (no LLM access)")
        _degraded_repl()
        return 0
...
def _degraded_repl() -> None:
    """REPL loop when core modules cannot be imported."""
    while True:
        try:
            user_input = input("arctus [degraded]> ").strip()
        except (EOFError, KeyboardInterrupt):
            print("\nGoodbye!")
            return
        ...
        print_error(f"Core modules unavailable: {_IMPORT_ERROR}")
        print_info("Fix the installation to enable full functionality")
...
def handle_config_command(raw: str) -> None:
    ...
    if len(parts) == 1:
        try:
            cfg = Config.load()
            # Use dict() conversion if to_dict() doesn't exist
...
def cmd_config(args: argparse.Namespace) -> int:
    ...
    try:
        ...
        config_path = Path.home() / ".config" / "arctus" / "config.json"
