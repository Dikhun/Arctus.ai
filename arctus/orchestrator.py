...
class QueenAgent:
    ...
    def plan(self, prompt: str, session_id: str = "") -> List[Step]:
        ...
        out = llm.chat(tier, [
            ...
            session_id=session_id if self.config.sticky_session_enabled else "",
        )
    ...
    def run_pipeline(self, prompt: str, session_id: str) -> TaskResult:
        ...
        and StateDir. Workers = self.config.max_workers (2/4/6 per tier).
    ...
    def run(
        ...
        # Meter tokens for the subscription tier.
        result.usage = llm.pop_usage()
        return result
