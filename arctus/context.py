...
class StateDir:
    ...
    def write_checkpoint(
        ...
        *,
        task_status: str = "",
        current_state_and_progress: str = "",
        pending_steps: str = "",
        blockers_edge_cases: str = "",
    ) -> Path:
        """Write a Context Offload Protocol handoff document.

        Implements the formal 80% offload protocol:
          1. Stop generation when context approaches ~80% capacity.
          2. Compile the structured handoff state (the kwargs below).
          3. Write it to this sandbox state file.
          4. The caller emits the final handoff notification message.

        Backward compatible: the original keys (what_has_been_done /
        what_needs_to_be_completed_next / saved_context) are preserved so
        existing readers keep working.
        """
        payload = {
            # ── Context Offload Protocol fields ──
            "protocol": "context-offload-v1",
            "task_status": task_status or done,
            "current_state_and_progress": current_state_and_progress or done,
            "pending_steps": pending_steps or next_steps,
            "blockers_edge_cases": blockers_edge_cases,
    ...
    def handoff_notification(self, state_file: Optional[Path] = None) -> str:
        """The final handoff message the agent returns to the supervisor."""
        sf = state_file or self.state_file
        return (
            f"Context threshold (80%) reached. Task state, progress, and "
            f"remaining instructions have been written to {sf}. "
            f"Ready for next agent invocation."
        )
