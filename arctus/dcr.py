"""DCR — Dynamic Context Router (lightweight local proxy).
...
for this step?" DCR scores candidates by keyword + recency + cost, then
returns the top-k.
"""
...
logger = logging.getLogger("arctus.dcr")
...
class DcrOptimizer:
    ...
    def _text_overlap(self, step_words: set, c: DcrCandidate) -> float:
        if not c.keywords:
            return 0.0
    ...
    def _recency(self, c: DcrCandidate, now: float) -> float:
        if c.last_used == 0.0:
            return 0.0
        age = max(0.0, now - c.last_used)
        return math.exp(-age / self.recency_half_life_s)

    def _cost_norm(self, c: DcrCandidate, max_cost: float) -> float:
    ...
    def rank(
        ...
        for c in candidates:
            ...
            results.append(DcrRank(id=c.id, score=score, parts={
                "text": text, "recency": rec, "cost": cost,
            }))
        results.sort(key=lambda r: r.score, reverse=True)
        for r in results[:top_k]:
            logger.debug("DCR rank %s = %.3f %s", r.id, r.score, r.parts)
        return results[:top_k]


# A registry of MCP tools / agent capabilities that DCR can rank.
# Populated by mcp.py and agents.py at startup.
_CAPABILITY_REGISTRY: List[DcrCandidate] = []


def register_capability(c: DcrCandidate) -> None:
    _CAPABILITY_REGISTRY.append(c)


def rank_for_step(step_text: str, top_k: int = 5) -> List[DcrRank]:
