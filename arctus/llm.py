...
class RunPodAdapter(BaseLLMClient):
    ...
    RunPod endpoints are OpenAI-compatible, so we reuse HTTPXAdapter patterns
    with RunPod-specific base URL and auth handling.
    """
    
    def __init__(self, *args: Any, **kwargs: Any):
        super().__init__(*args, **kwargs)
        if httpx is None:
            raise LLMConfigError("httpx is required for RunPodAdapter. "
                                 "Install: pip install httpx")
        self.client = httpx.AsyncClient(
            base_url=self.preset.base_url,
            timeout=httpx.Timeout(self.preset.request_timeout),
    ...
    async def chat(
        ...
        """
        RunPod serverless endpoints use OpenAI-compatible /chat/completions.
        Delegates to the same pattern as HTTPXAdapter.
        """
        payload: dict[str, Any] = {
            "model": self.model.model_id,
            "messages": [
        ...
        # Retry loop with RunPod-specific handling
        last_error: Optional[Exception] = None
        for attempt in range(self.preset.max_retries + 1):
            try:
                if stream:
                ...
                wait = self.preset.retry_backoff * (2 ** attempt)
                logger.warning("RunPod rate limited; retrying in %.1fs (attempt %d/%d)",
                             wait, attempt + 1, self.preset.max_retries)
                await asyncio.sleep(wait)
            except (LLMRequestError, httpx.RequestError) as exc:
                last_error = exc
        ...
        raise LLMRequestError(f"RunPod failed after {self.preset.max_retries} retries: {last_error}")
...
def create_client(
    ...
    """
    Factory that resolves presets and returns the appropriate adapter.
    """
    preset = resolve_preset(provider)
    model_preset = preset.get_model(model) if model else preset.default_model
    
    if preset.name == "ollama":
        return OllamaAdapter(preset, model_preset, api_key)
    elif preset.name == "huggingface":
        return HuggingFaceAdapter(preset, model_preset, api_key)
    elif preset.name == "runpod":
        return RunPodAdapter(preset, model_preset, api_key)
    else:
        # OpenRouter, OmniRoute, OpenAI-compatible
        return HTTPXAdapter(preset, model_preset, api_key)
...
class LLMClient:
    """
    Synchronous wrapper around the async adapters.
    """
    ...
    def __init__(
        ...
        self._async_client = create_client(provider, model, api_key)
    ...
    def chat(
        ...
        if stream:
            # For streaming, return an async iterator wrapped for sync use
            # Caller should use `async for` or we provide an async API
            return coro  # type: ignore[return-value]
        return loop.run_until_complete(coro)  # type: ignore[return-value]
    
    def embeddings(self, texts: list[str]) -> list[list[float]]:
        loop = self._get_loop()
        return loop.run_until_complete(self._async_client.embeddings(texts))
    
    def close(self) -> None:
        if self._loop and not self._loop.is_closed():
            if hasattr(self._loop, "is_running") and not self._loop.is_running():
