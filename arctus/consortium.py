"""Consortium — peer-to-peer work distribution across Arctus instances.
...
logger = logging.getLogger("arctus.consortium")
...
class ConsortiumHub:
    """In-process registry + queue for consortium peers."""

    def __init__(self) -> None:
        self.peers: Dict[str, Peer] = {}
        self.queue: List[ConsortiumTask] = []
        self.results: Dict[str, ConsortiumTask] = {}
    ...
    def add_peer(self, peer: Peer) -> None:
        ...
        logger.info("Consortium peer added: %s -> %s", peer.name, peer.base_url)
    ...
    def submit_local(self, task: ConsortiumTask) -> None:
        with self._lock:
            self.queue.append(task)

    def claim_next(self) -> Optional[ConsortiumTask]:
        with self._lock:
            for t in self.queue:
                if t.status == "pending":
                    t.status = "running"
    ...
    def push_to_peer(self, peer_name: str, task: ConsortiumTask) -> bool:
        """Push a task to a remote peer over HTTP."""
        peer = self.peers.get(peer_name)
        if not peer or not peer.enabled:
            return False
        url = peer.base_url.rstrip("/") + "/api/consortium/submit"
        body = json.dumps({
    ...
    def pull_result_from_peer(self, peer_name: str, task_id: str) -> Optional[Any]:
        ...
        if not peer or not peer.enabled:
            return None
        url = f"{peer.base_url.rstrip('/')}/api/consortium/result/{task_id}"
        req = urllib.request.Request(
            url, method="GET",
